...
def convert_added_at(db_path):
    ...
    print("Đang chuyển đổi release_date trong bảng javtiful_videos...")
    cursor.execute("SELECT id, release_date FROM javtiful_videos WHERE release_date IS NOT NULL AND release_date != ''")
    rows = cursor.fetchall()
    count_rd = 0
    for row_id, release_date in rows:
        new_rd = parse_release_date(release_date)
        if new_rd != release_date:
            cursor.execute("UPDATE javtiful_videos SET release_date = ? WHERE id = ?", (new_rd, row_id))
            count_rd += 1
    print(f"Đã cập nhật {count_rd} bản ghi release_date trong javtiful_videos.")
